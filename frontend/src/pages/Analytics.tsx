...
const Analytics: React.FC = () => {
  ...
  const aggregateBarData = aggregates ? [
    ...
    { name: 'Posture', score: aggregates.averagePosture },
    { name: 'Speech Clarity', score: aggregates.averageClarity },
    { name: 'Accuracy', score: aggregates.averageAccuracy }
  ] : [];
        ...
        <main className="flex-grow p-8 space-y-8 max-w-6xl w-full mx-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-500">
              ...
              <span>Analyzing historical metrics...</span>
            </div>
          ) : history.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 glass-card rounded-2xl p-6 border border-slate-800 space-y-4">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">Your average rating progress charted across sequential mock sessions.</p>
                  </div>
                  ...
                  <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={trend} margin={{ left: -20, right: 10 }}>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: '#1E293B', borderColor: '#334155', borderRadius: '8px', color: '#F8FAFC', fontSize: '11px' }} />
                ...
                <div className="glass-card rounded-2xl p-6 border border-slate-800 space-y-4">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">Averages metrics across all speech and posture checks.</p>
                  </div>

                  <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={aggregateBarData} margin={{ left: -20, right: 10 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#1E293B" />
              ...
              <div className="glass-card rounded-2xl p-6 border border-slate-850 space-y-4">
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-850">
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-slate-900/60 text-slate-400 border-b border-slate-850 font-bold uppercase">
                    ...
                    <tbody className="divide-y divide-slate-850 bg-slate-950/20 text-slate-300">
                      {history.map((session) => (
                        <tr key={session._id} className="hover:bg-slate-900/40 transition-colors">
                          ...
                          <td className="p-4 font-extrabold text-[#A78BFA]">
                            {session.status === 'completed' ? `${session.overallScore}/100` : 'Not Completed'}
                          </td>
